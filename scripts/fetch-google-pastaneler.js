...
const OUTPUT_PATH = path.join(DATA_DIR, 'pastaneler.json');
const BACKUP_PATH = path.join(DATA_DIR, 'pastaneler.backup.json');
...
const SEARCH_TERMS = String(process.env.SEARCH_TERMS || 'pastane')
  .split(',')
  .map((item) => item.trim())
  .filter(Boolean);
...
function looksLikePastryShop(place) {
  ...
  return (
    ...
    name.includes('tatlici') ||
    name.includes('tatlıcı')
  );
}
...
function buildQueryList(districtMap, terms) {
  ...
  Object.entries(districtMap).forEach(([city, districts]) => {
    ...
    districtList.forEach((district) => {
      const districtName = normalizeText(district || 'Merkez');
      if (!districtName) {
        return;
      }
...
function saveDataset(existingList, finalList) {
  if (DRY_RUN) {
    console.log('Dry-run: pastaneler.json yazilmadi.');
...
function saveSnapshot(venueMap) {
  ...
  const snapshot = sortPastaneler(venueMap.values());
...
async function run() {
  if (!API_KEY) {
    console.error('PLACES_API_KEY bulunamadi. Ornek: PLACES_API_KEY=... node scripts/fetch-google-pastaneler.js');
    process.exitCode = 1;
    return;
  }

  if (SEARCH_TERMS.length === 0) {
    console.error('SEARCH_TERMS bos olamaz. Ornek: SEARCH_TERMS=pastane,bakery');
    process.exitCode = 1;
    return;
  }
  ...
  for (let index = 0; index < activeQueries.length; index += 1) {
    ...
    if ((index + 1) % 20 === 0 || index === activeQueries.length - 1) {
      console.log(
        `Ilerleme: ${index + 1}/${activeQueries.length} (global=${safeStart + index + 1}/${queryList.length}) | Ham sonuc: ${rawResultCount} | Eslesen: ${acceptedCount} | Esiz pastane: ${venueMap.size} | Istek: ${requestCount}`,
      );
    }
  ...
  const finalList = sortPastaneler(venueMap.values());

  if (finalList.length === 0) {
    console.error('Kayit bulunamadi. pastaneler.json guncellenmedi.');
    process.exitCode = 1;
    return;
  }
