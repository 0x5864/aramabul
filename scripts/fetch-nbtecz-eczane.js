...
const OUTPUT_PATH = path.resolve(ROOT, "data/eczane.json");
const BACKUP_PATH = path.resolve(ROOT, "data/eczane.backup.json");

const ROOT_URL = "https://www.nbtecz.com.tr/Eczaneler";
  ...
  try {
    const response = await fetch(url, {
      headers: {
        ...
        "User-Agent": "arama-bul/1.0 (+https://www.nbtecz.com.tr/Eczaneler)",
      },
...
function extractCityUrls(rootHtml) {
  const cityUrlSet = new Set();
  const regex = /href="(\/Eczaneler-[a-z0-9-]+)"/gi;

  for (const match of rootHtml.matchAll(regex)) {
    const relative = String(match[1] || "").trim();
    if (!relative || relative === "/Eczaneler") {
      continue;
    }
    cityUrlSet.add(`${BASE_URL}${relative}`);
  }

  return [...cityUrlSet].sort((a, b) => a.localeCompare(b, "tr"));
}
...
function extractCategoryItems(blockHtml) {
  ...
  for (const match of categoryHtml.matchAll(liRegex)) {
...
function extractEczaneRecords(cityHtml) {
  ...
  blocks.forEach((block) => {
    ...
    const mapsUrl = cleanText(addressMatch && addressMatch[1], 500);
...
async function main() {
  ...
  if (cityUrls.length === 0) {
    throw new Error("Sehir URL listesi bulunamadi.");
  }

  const allRecords = [];
  let failed = 0;

  process.stdout.write(`Sehir sayfasi: ${cityUrls.length}\n`);
  ...
  for (let i = 0; i < cityUrls.length; i += 1) {
    ...
    try {
      ...
      const records = extractEczaneRecords(html);
  ...
  if (finalRecords.length === 0) {
    throw new Error("Hic kayit cikmadi.");
  }
