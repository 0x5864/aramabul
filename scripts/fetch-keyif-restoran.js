...
const OUTPUT_PATH = path.join(DATA_DIR, "keyif-restoran.json");
const BACKUP_PATH = path.join(DATA_DIR, "keyif-restoran.backup.json");
const SOURCE_URL = "https://www.bulurum.com/dir/restoranlar/";
...
function parseTotalCount(html) {
  const totalMatch = html.match(/mainCountResultsTitle[^>]*>\s*:\s*([0-9\.\,]+)\s*sonuç/i);
  if (!totalMatch) {
    return 0;
  }
...
function isCaptchaPage(html) {
  const hasResults = html.includes("mainCountResultsTitle") || html.includes("mapInfo.results =");
  if (hasResults) {
    return false;
  }
...
function mapResultToVenue(result, districtMap, districtIndex, expectedCity = "") {
  ...
  const detailUrl = normalizeDetailUrl(result?.DetailUrl);
...
function dedupeVenues(venues) {
  ...
  for (const venue of venues) {
    const key = normalizeKey(
      venue.detailUrl ||
        `${venue.city}|${venue.district}|${venue.name}|${venue.address}|${venue.mapsUrl}`,
    );
    if (!key || seen.has(key)) {
      continue;
    }
...
async function main() {
  ...
  for (let cityIndex = 0; cityIndex < targetCities.length; cityIndex += 1) {
    ...
    const firstRows = parseResultsArray(cityFirstResponse.html);
    ...
    for (let pageIndex = START_PAGE; pageIndex < cityPageLimit; pageIndex += 1) {
      ...
      const rawRows = parseResultsArray(html);
