...
const OUTPUT_FILE = path.join(ROOT, "data", "kultur-camiler.json");
...
const FALLBACK_CAMI_TUR_KOD = "9";
const REQUEST_TIMEOUT_MS = 45000;
const REQUEST_RETRY_COUNT = 3;
const REQUEST_RETRY_DELAY_MS = 450;
const CITY_CONCURRENCY = 6;
const SOURCE_LABEL = "Kaynak: kulturportali.gov.tr harita API (Gezilecek Yerler > Cami / Tarihi Camiler)";
  ...
  for (let attempt = 1; attempt <= REQUEST_RETRY_COUNT; attempt += 1) {
    ...
    try {
      const response = await fetch(url, {
        ...options,
        headers: {
          "User-Agent": "aramabul-kultur-cami-fetcher/1.0 (+https://aramabul.com)",
...
async function fetchCamiTurKodu() {
  ...
  const found = categories.find((item) => {
    const title = normalizeForLookup(item && item.Ad);
    return title === "cami" || title === "cami̇" || title.includes("cami");
  });
  return normalizeText(found && found.Kod) || FALLBACK_CAMI_TUR_KOD;
}
...
async function fetchCamiRowsForCity(cityId, turKod) {
  const payload = await fetchJsonWithRetry(
...
async function main() {
  ...
  const turKod = await fetchCamiTurKodu();
  console.log(`==> İl sayısı: ${cities.length}`);
  console.log(`==> Cami tur kodu: ${turKod}`);
  ...
  const dataResults = await mapWithConcurrency(
    citiesWithIds,
    async (city) => {
      const rows = await fetchCamiRowsForCity(city.cityId, turKod);
      return { city, rows };
    },
    CITY_CONCURRENCY,
  );
  ...
  writeOutput(unique);
  console.log(`==> Toplam cami kaydı: ${unique.length}`);
  console.log(`==> Yazıldı: ${OUTPUT_FILE}`);
  console.log(`==> Yazıldı: ${ANDROID_OUTPUT_FILE}`);
}

main().catch((error) => {
  console.error("fetch-kultur-camiler hata:", error && error.message ? error.message : error);
  process.exitCode = 1;
});
