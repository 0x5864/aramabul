...
const OUTPUT_FILE = path.join(ROOT, "data", "kultur-oren-yerleri.json");
...
const FALLBACK_OREN_TUR_KOD = "47";
const REQUEST_TIMEOUT_MS = 45000;
const REQUEST_RETRY_COUNT = 3;
const REQUEST_RETRY_DELAY_MS = 450;
const CITY_CONCURRENCY = 6;
const SOURCE_LABEL = "Kaynak: kulturportali.gov.tr harita API (Gezilecek Yerler > Ören Yeri)";
  ...
  for (let attempt = 1; attempt <= REQUEST_RETRY_COUNT; attempt += 1) {
    ...
    try {
      const response = await fetch(url, {
        ...options,
        headers: {
          "User-Agent": "aramabul-kultur-oren-fetcher/1.0 (+https://aramabul.com)",
...
async function fetchOrenTurKodu() {
  ...
  const found = categories.find((item) => normalizeForLookup(item && item.Ad).includes("oren yeri"));
  return normalizeText(found && found.Kod) || FALLBACK_OREN_TUR_KOD;
}
...
async function fetchOrenRowsForCity(cityId, turKod) {
  const payload = await fetchJsonWithRetry(
...
async function main() {
  ...
  const turKod = await fetchOrenTurKodu();
  console.log(`==> İl sayısı: ${cities.length}`);
  console.log(`==> Ören Yeri tur kodu: ${turKod}`);
  ...
  const dataResults = await mapWithConcurrency(
    citiesWithIds,
    async (city) => {
      const rows = await fetchOrenRowsForCity(city.cityId, turKod);
      return { city, rows };
    },
    CITY_CONCURRENCY,
  );
  ...
  writeOutput(unique);
  console.log(`==> Toplam ören yeri kaydı: ${unique.length}`);
  console.log(`==> Yazıldı: ${OUTPUT_FILE}`);
  console.log(`==> Yazıldı: ${ANDROID_OUTPUT_FILE}`);
}

main().catch((error) => {
  console.error("fetch-kultur-oren-yerleri hata:", error && error.message ? error.message : error);
  process.exitCode = 1;
});
