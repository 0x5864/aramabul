const ASSET_VERSION = "20260225-01";
...
async function loadDistrictMap() {
  if (!districtMapPromise) {
    ...
    districtMapPromise = fetch(withVersion(DISTRICTS_JSON_PATH), { cache: "no-store" })
      .then((response) => {
        if (!response.ok) {
          return {};
        }
...
async function loadAllVenues() {
  if (!allVenuesPromise) {
    allVenuesPromise = fetch(withVersion(CATEGORY_VENUES_JSON_PATH), { cache: "no-store" })
      .then((response) => {
        if (!response.ok) {
          throw new Error(`Mekan verisi alınamadı: ${response.status}`);
        }
...
async function loadCategoryDataFile(dataFilePath) {
  ...
  if (!categoryDataPromiseCache.has(cacheKey)) {
    const fallback = readFallbackData();
    if (fallback && cacheKey.includes("akaryakit") && Array.isArray(fallback.akaryakit)) {
      categoryDataPromiseCache.set(cacheKey, Promise.resolve(fallback.akaryakit));
      return categoryDataPromiseCache.get(cacheKey);
    }

    const promise = fetch(withVersion(cacheKey), { cache: "no-store" })
      .then((response) => {
        if (!response.ok) {
          return [];
        }
...
function renderRootPage(definition, venues, districtMap = null) {
  ...
  const citySet = definition.useDistrictCatalog && districtMap
    ? new Set(Object.keys(districtMap))
    : new Set(venues.map((venue) => venue.city));
  const groups = regionGroupsForCities(citySet);
...
function renderCityPage(definition, venues, districtMap = null) {
  ...
  const cityNames = (
    definition.useDistrictCatalog && districtMap
  ...
  const districts = (
    definition.useDistrictCatalog && districtMap
      ? [...new Set((districtMap[matchedCity] || []).map((value) => String(value || "").trim()).filter(Boolean))]
      : [...new Set(venues
...
function renderDistrictPage(definition, venues, districtMap = null, secondaryVenues = []) {
  ...
  const districtVenues = dedupeByName(
    venues.filter((venue) => venue.city === matchedCity && venue.district === matchedDistrict),
  );
  const districtSecondaryVenues = dedupeByName(
    secondaryVenues.filter((venue) => venue.city === matchedCity && venue.district === matchedDistrict),
  );
