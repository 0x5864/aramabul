...
const KUAFOR_JSON_PATH = `data/kuafor.json?v=${ASSET_VERSION}`;
...
async function loadKuaforPlaces() {
  ...
  const response = await fetch(KUAFOR_JSON_PATH, { cache: "no-store" });

  if (!response.ok) {
    throw new Error(`Kuaför verisi alınamadı: ${response.status}`);
  }
